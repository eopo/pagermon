 ...
 */
function create() {
    worker = new Worker('sdr-messages', async job => {
        const message = job.data;
        
        // Ensure message has required fields (backward compatibility with old jobs)
        if (!message.address || !message.message) {
            console.error('[WORKER] Invalid message format (missing address or message):', message);
        ...
        try {
            ...
            if (!response.ok) {
                ...
                if (isTransient) {
                    ...
                    await queue.moveToDeadLetterQueue(
                        job.id,
                        message,
                        error.message,
                        job.attemptsMade
